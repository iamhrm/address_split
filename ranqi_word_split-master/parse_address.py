...
# 删除字符串的特定字符
def remove_province_char_if_exist(string, char):
	if string[:3] == char:
		return string[3:]
	return string
...
def parse_address(input_filename, output_filename=""):
	...
	for input_line in input_lines:
		...
		#0 替换福建省
		working_line= remove_province_char_if_exist(working_line, "福建省") # 去除额外的"福建省"字
		...
		for district in address_dict.districts:
			...
			if working_line.find(district,0,5) != -1: # 找到区划
				...
				working_line = remove_initial_char_if_exist(working_line, "县")  # 去除额外的"县"字
				break # 找到区划即可结束找区循环
...
if __name__ == "__main__":
	...
	parse_address("test.txt", "output_test1.txt")